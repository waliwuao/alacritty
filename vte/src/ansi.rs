...
pub trait Handler {
    ...
    ///
    /// `kind` is one of: b'A' (prompt start), b'B' (command start),
    /// b'C' (command end/output start), b'D' (exit code).
    fn shell_integration_kind(&mut self, _kind: u8) {}
}
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            b"133" if params.len() >= 2 && params[1].len() == 1 => {
                self.handler.shell_integration_kind(params[1][0]);
            },
